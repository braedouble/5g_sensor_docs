...
class ICM42688
{
  public:
     ...
     */
    ICM42688(SPIClass &bus, uint8_t csPin, uint32_t SPI_HS_CLK=24000000);
    ...
    int enableFifoThresholdInterrupt();
    int getFifoSize();
    bool setFifoSize(uint16_t fifoSize);
     ...
     */
    int getAGT();
    void IRAM_ATTR getRawAGT();
    void IRAM_ATTR getRawAGT(int *x, int *y, int *z);
  ...
  protected:
    ...
    // 14.45 FIFO_CONGIG1
    const uint8_t FIFO_EN = 0x5F; // FIFO_CONFIG1 ADDR
    const uint8_t FIFO_HIGHRES_EN = 0x16; // bit 4 1 = EN High Res
    const uint8_t FIFO_TEMP_EN = 0x04; // bit 2 1 = EN Temp
    const uint8_t FIFO_GYRO = 0x02; // bit 1 1 = EN Gyro
    const uint8_t FIFO_ACCEL = 0x01; // bit 0 1 = EN Accel
    ...
    int IRAM_ATTR readRegisters(uint8_t subAddress, uint8_t count, uint8_t *dest);
...
class ICM42688_FIFO: public ICM42688 {
  public:
    ...
    int enableFifo(bool accel,bool gyro, bool highRes);
    int streamToFifo();
    int readFifo();
    int getFifoSize();
    bool setFifoSize(uint16_t fifoSize);
    void getFifoAccelX_raw(size_t *size,int* data); // int is int32 by default
    void getFifoAccelY_raw(size_t *size,int* data);
    void getFifoAccelZ_raw(size_t *size,int* data);
    ...
    void getFifoHeader(size_t *size,uint8_t* data);
  protected:
    // fifo
